...
In this basic example, the most likely tercile (i.e., the one with the highest probability, or in other words, the one predicted by the highest number of ensemble members) is indicated by the color of the bubbles. All bubbles have equal size.
...
The `score.range` argument is useful in order to mask from the map all information deemed non-relevant or visually distracting depending on each user application. This is a vector of length two used to rescale the transparency of the bubbles. For instance, a `score.range = c(0.5, 0.8)` will turn ROCSS values below 0.5 completely transparent, while values of 0.8 or more have minimum transparency, i.e., opaque). The default is `NULL`, setting a transparency range between 0 and 1, as in the previous plot.
...
Another variant of bubble plots consists in replacing the bubbles by pie charts. As a result, instead of just the most likely tercile, the probabilities for each tercile are given: each of the three sectors of the pie-chart provide a quantitative measurement of the proportion of members falling in each tercile. The terciles are identified by colors (i.e., red upper, grey middle, blue lower tercile). The main advantage of this display is that it allows for the visualization of all terciles simultaneously, instead of just the most likely one, as with the ordinary bubbles. Furthermore, as in the previous examples, the ROCSS can be equally represented by transparency, and masking is also possible using the `score.range` argument, as before. This is represented in the next plot. Note that in order to have a better view of the pie charts, a domain centered in the North Atlantic is subset prior to plotting (thuis is achieved through the `transformeR::subsetGrid` function):
